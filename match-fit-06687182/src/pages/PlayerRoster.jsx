...
import { Users, Shield, Target, Zap, TrendingUp, Lock, User as UserIcon, ArrowLeft, Grid3x3, List } from "lucide-react";
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (membersError) {
        ...
        // No need for fallback RPC call since emails are synced to profiles table
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#e7f3fe] via-white to-[#e7f3fe] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-lg p-12 text-center max-w-2xl mx-auto"
          >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12 space-y-8">
          ...
          <div>
            ...
            <p className="text-lg">
              <span className="bg-gradient-to-r from-[#118ff3] to-[#0c5798] bg-clip-text text-transparent font-semibold">
                {view === "coaches" ? "View your coaching team" : view === "players" ? "View your teammates" : "View your team members"}
              </span>
            </p>
          </div>
        ...
        {view === "players" && (
          <motion.div
            ...
            className="space-y-6"
          >
            {/* Header with View Toggle and Color Guide */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-0.5">{allPlayers.length} {allPlayers.length === 1 ? 'player' : 'players'}</p>
                </div>
              </div>
            ...
            {hasPlayers ? (
              viewMode === "card" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {allPlayers.map((player, index) => renderPlayerCard(player, index))}
                </div>
              ) : (
                <div className="space-y-2">
                  {allPlayers.map((player, index) => renderPlayerListRow(player, index))}
                </div>
              )
            ) : (
              <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-200/50 shadow-lg p-12 text-center">
                ...
                <p className="text-slate-600">There are no players on your team yet.</p>
              </div>
            )}
          </motion.div>
        )}
